...
loomground-solver/
  ...
  README.md  docs/
```
(The RVND-side glue — the shims and RVND adapters — lives in the RVND repo at
...
- `tests/test_api_parity.py` — every public symbol RVND's modules exposed is re-exported by the matching package module.

## Wire RVND to import it — and lose nothing (run this in your Terminal)

The RVND glue lives in the RVND repo at `loomground-solver-integration/`. Its `rvnd_shims/workspaces/*.py` replace six RVND module bodies with re-exports from the package, so every old `workspaces.reasoning* / norm_contract / dimensions / solver_topology` import keeps resolving; `reasoning_contract`'s `check_folder_case` is kept in the shim (it still reads `policy`), so RVND behaviour is identical.

```bash
cd /path/to/RVND/server
...
python3 -m pytest -q                                     # THE FINAL GATE: full RVND suite green
```
...
Versum imports the same package as its L3 apex. Implement `NormSource` over the claim layer (join on `canonical_urn`) and call `entail(...)` / `check(..., governance=<terrain-write>)`. Nothing in `versum/` imports the solver's internals — same one-way, port-only contract as RVND.

## Adapters — how the KG and RVND plug in
...
In the **RVND** repo (`loomground-solver-integration/adapters_rvnd/`), with their tests:
- `RvndNormSource` — `NormSource` over `workspaces.rule_registry.RuleRegistry`.
- `RvndGovernance` — `Governance` over RVND: oversight from `policy.load_policy` (level + opt-out), custody from `lock_classify`, audit via `mutation_log.append_raw`; lazy host imports, every dep injectable.
...
- **Corpus-coupled functions deferred:** `problem_kg.build_case` / `gate_case` / `derive_actions`, the `reasoning_walker`, and `check_folder_case` stay on the RVND side (they need `rule_registry` / `legal_corpus` / `policy`). The package ships the pure `CaseRecord` model and the ports they will plug into.
