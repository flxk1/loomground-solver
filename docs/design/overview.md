# Kernel overview, adapter history and compatibility

Text moved verbatim from the README (2026-09-09); the README keeps inputs, outputs, ports and dependency direction, this file keeps the narrative. Figures quoted below are as of that README.
...
[`reasoning.interop` 1.0 contract](../contracts/reasoning-interop.md), while remaining fully standalone. Versum is
one conforming graph, not a core dependency.

Loomground language support and its explicit version policy are documented in
[Loomground compatibility policy](../contracts/loomground-compatibility.md).
Cross-repository release order and automated version administration are defined
in the [release and versioning guide](../guides/releasing.md).
...
The [vision](../design/vision.md) states the intent this code serves; the
[operations guide](../guides/operations.md) explains how to run it
